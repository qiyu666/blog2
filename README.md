...
- **Markdown 编辑器快捷键**（Ctrl+B 加粗 / Ctrl+I 斜体 / Ctrl+K 插入链接 / Ctrl+Q 引用 / Ctrl+U 无序列表 / Ctrl+1~5 标题）
- 文章草稿与发布状态管理
- 分类、标签、封面图
...
- **文章字体族切换**（系统/无衬线/衬线/等宽四种字体族，localStorage 持久化）
...
- **引用回复**（评论操作栏新增"引用"按钮，自动插入 > 格式）
- 文章点赞与收藏
- **点赞排行榜**（侧边栏热门文章 TOP5，金银铜徽章样式）
- 用户关注 / 粉丝体系
- **关注列表 Feed**（/following 页面，游标分页，展示关注用户的最新文章）
- 站内私信
...
- **深色模式增强**（三态切换：light/dark/auto，系统主题监听）
...
| GET | `/api/following` | 关注列表 Feed（游标分页） |
